...
class UrlCrawler:
    ...
    def __init__(self, site_base_url, max_threads=0, logging_enabled=False):
        """
        Constructor for UrlCrawler.
        :param site_base_url:
        :param max_threads:
...
class CrawlerThread(threading.Thread):
    ...
    def find_all_urls_on_page(self, url, use_callback=True):
        ...
        for a in soup.findAll("a"):
            ...
            if self.mode == UrlCrawler.Mode.INTERNAL:
                if not self.is_internal_url(a_href):
                    continue
            elif self.mode == UrlCrawler.Mode.EXTERNAL:
                if self.is_internal_url(a_href):
                    continue
            elif self.mode == UrlCrawler.Mode.ALL:
                pass
